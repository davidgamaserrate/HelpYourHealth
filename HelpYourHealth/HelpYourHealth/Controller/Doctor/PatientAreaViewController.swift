...
class PatientAreaViewController: UIViewController, UIGestureRecognizerDelegate, UINavigationControllerDelegate {
    ...
    var names: [String] = ["Histórico Médico", "Consultas", "Exames", "Laudos", "Medicamentos", "Sintomas"]
...
extension PatientAreaViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 { DispatchQueue.main.async { self.performSegue(withIdentifier: "HistoricViewController", sender: self) } }
        if indexPath.row == 4 { DispatchQueue.main.async { self.performSegue(withIdentifier: "MedicinesViewController", sender: self) } }
    }
}
